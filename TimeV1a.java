package TimeClass.version1a ;
public class TimeV1a implements  Comparable<TimeV1a> {
    //Constructors
    private int hour;
    private int min;
    private int sec;

    public TimeV1a() { // create midnight
        hour=min=sec=0;
    }

    public TimeV1a(int h, int m, int s) {
        this.hour=h;
        this.min=m;
        this.sec=s;
    }

    public TimeV1a(TimeV1a t) {
        this.hour=t.hour;
        this.min=t.min;
        this.sec=t.sec;
    ...
    public int compareTo(TimeV1a t) { //produces the number of seconds from t to this. That is, t.advanceBy(this.compareTo(t)) should set t to the same time as this.
    ...
    public boolean equals(TimeV1a another) { //returns true if and only if the times have the same value.
